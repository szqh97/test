...
int video_split_processor::video_split(FILE *fp, off_t pos, video_file_info *p_info)
{
    ...
    else
    {
        ...
        if (m_curr_shot.start > 0 and m_curr_shot.end > 0)
        {
            cout << "---shot: " <<m_curr_shot.start << ", " << m_curr_shot.end << ", " << m_curr_shot.start_frame_type << ", " << m_curr_shot.end_frame_type << endl;
            if (m_fp)
            {
                fclose(m_fp);
                m_fp = NULL;
            ...
            tv.tv_usec = (p_info->begin_time.tv_usec + fts*1000) % 1000000;
