...
def start(port, root_directory="/tmp/s3", bucket_depth=0):
    ...
    size = 5 * 2**10 * 2**10 * 2**10
    http_server = httpserver.HTTPServer(application, chunk_size = size, max_buffer_size = size)
...
class MetadnaHandler(BaseRequestHandler):
    def get(self, id1, id2, objectname):
        raise web.HTTPError(503)
        self.write('xxxxx' + str(objectname))
...
class RootHandler(BaseRequestHandler):
    ...
    @gen.engine
    def get(self):
        s = yield gen.Task(ioloop.IOLoop.instance().add_timeout, time.time() + 5)
        names = os.listdir(self.application.directory)
        buckets = []
        for name in names:
            path = os.path.join(self.application.directory, name)
        ...
        info = {"ListAllMyBucketsResult":{"Buckets":{"Buckets":buckets}}}
