...
#print 'usage: ', sys.argv[0], "sample_site_view_count.csv sample_site_basic.csv estimated_basic.csv"
def usage():
    """    
    ...
    """    
    print 'usage: ', sys.argv[0], "new_sample_site_info.csv old_sample_site_info.csv matched_videos.csv arch_matched_videos.csv"
...
def new_title_based_piracy(new_sample_site_info):
    ...
    new_sample_site_info['pageview_percent']  = new_sample_site_info.pageview_percentage.apply(lambda x: float(x.replace('%', ''))/100.0)
    new_sample_site_info['title_based_piracy'] = new_sample_site_info.view_count/(new_sample_site_info.daily_global_pageview * new_sample_site_info.cr * new_sample_site_info.pageview_percent)

    if len(new_sample_site_info) >5:
        new_sample_site_info = new_sample_site_info.sort(columns = 'title_based_piracy')[1:-1]


    avg_title_based_piracy = (new_sample_site_info.view_count * new_sample_site_info.title_based_piracy).sum()/new_sample_site_info.view_count.sum()
...
def old_title_based_piracy(old_sample_site_info, estimated_site_match):
    ...
    old_sample_site_info['pageview_percent']  = old_sample_site_info.pageview_percentage.apply(lambda x: float(x.replace('%', ''))/100.0)
    old_sample_site_info['title_based_piracy'] = old_sample_site_info.view_count/(old_sample_site_info.daily_global_pageview * old_sample_site_info.cr * old_sample_site_info.pageview_percent)
...
def merge_matched_videos(matchedvideos, arch_matchedvideos):
    ...
    g = tmp_m.groupby('trackingWebsite_id')
    merged_matchedvideos = pd.concat([g.website_type.all(), g.matches.sum()], axis = 1)
    merged_matchedvideos['trackingWebsite_id'] = merged_matchedvideos.index
    return merged_matchedvideos


def gen_new_mediated_data(new_sample_site_info_table, matched_videos_table, arch_matched_videos_table, meta_id):
    ...
    pd_merged_matched_videos = pd_merged_matched_videos.merge(new_dailySiteViewBasic, on='trackingWebsite_id')
...
def gen_old_mediated_data(old_sample_site_info_table, matched_videos_table, arch_matched_videos_table, meta_id):
    ...
    if matched_videos_table == None or arch_matched_videos_table == None:
        ...
        pd_arch_matched_videos = pd.read_table(arch_matched_videos_table)
        pd_merged_matched_videos = merge_matched_videos(pd_matched_videos, pd_arch_matched_videos)
    pd_merged_matched_videos = pd_merged_matched_videos.merge(old_dailySiteViewBasic, on='trackingWebsite_id')

    def lambda_old_title_based_piracy(matches):
        return old_title_based_piracy(pd_old_sample_site_info, matches)

    pd_merged_matched_videos['title_based_piracy'] = pd_merged_matched_videos.matches.apply(lambda_old_title_based_piracy)
    pd_merged_matched_videos['trackingMeta_id'] = meta_id
    return pd_merged_matched_videos.query("website_type in ('ugc', 'hybrid')")
...
def main_func():
    ...
    with file(new_mediated_data_file, 'a') as f:
        f.write(header)
    with file(old_mediated_data_file, 'a') as f:
        f.write(header)
    
    with file (tracking_meta_files, 'r') as fd_trackingMetas:
        meta_id = fd_trackingMetas.readline().replace('\n','')
        while len(meta_id) !=0:
            ...
            if not os.path.exists(new_sample_site_info_table) and os.path.exists(old_sample_site_info_table) and os.path.exists(matched_videos_table) and os.path.exists(arch_matched_videos_table):
                print >> sys.stderr, meta_id, " files errror!"
                meta_id = fd_trackingMetas.readline().replace('\n','')
                continue
            if os.stat(new_sample_site_info_table).st_size == 0 or os.stat(old_sample_site_info_table).st_size == 0:
                print >> sys.stdout, meta_id, " no sample files"
                meta_id = fd_trackingMetas.readline().replace('\n','')
                continue
            if os.stat(matched_videos_table).st_size == 0 and os.stat(arch_matched_videos_table).st_size == 0:
            ...
            new_consumption_data = gen_new_mediated_data(new_sample_site_info_table, matched_videos_table, arch_matched_videos_table, meta_id)
            old_consumption_data = gen_old_mediated_data(old_sample_site_info_table, matched_videos_table, arch_matched_videos_table,meta_id)
...
#   new_mediated_data_file = 'new_consumption_file.csv'
#   old_mediated_data_file = 'old_consumption_file.csv'
#   new_consumption_data = gen_new_mediated_data(new_sample_site_info_table, matched_videos_table, arch_matched_videos_table)
#   old_consumption_data = gen_old_mediated_data(old_sample_site_info_table, matched_videos_table, arch_matched_videos_table)
